...
class AI_Pose_Estimator:
    ...
    def render_snapshot(self, points, image_size=1024):
        ...
        params = {
            "scale": scale, "center_u": center_u, "center_v": center_v,
    ...
    def get_rotation_matrices(self):
        ...
        labels = []
        # Rotații standard
        matrices.append(np.eye(3)); labels.append("Original")
        matrices.append(np.array([[1,0,0],[0,0,-1],[0,1,0]])); labels.append("Rot_X_90")
        matrices.append(np.array([[1,0,0],[0,0,1],[0,-1,0]])); labels.append("Rot_X_-90")
        matrices.append(np.array([[0,0,1],[0,1,0],[-1,0,0]])); labels.append("Rot_Y_90")
        matrices.append(np.array([[0,-1,0],[1,0,0],[0,0,1]])); labels.append("Rot_Z_90")

        # ADAUGĂ rotații la 45° (mai multe șanse):
        import math
    ...
    def predict(self, pcd, real_height_meters=1.75):
        ...
        # 1. Găsim orientarea (Brute Force)
        for RotMat, label in self.get_rotation_matrices():
            ...
            if results.pose_landmarks:
                ...
                # SCORING NOU - PRIORITATE PENTRU ORIENTARE VERTICALĂ REALĂ
                base_score = np.mean([lm.visibility for lm in lms])
                
                # ============ SECȚIUNEA NOUĂ ============
                # Verificăm orientarea REALĂ în spațiu 3D (nu în imagine!)
                # Calculăm înălțimea efectivă pe axa Y (verticala în spațiu rotit)
                height_in_rotated_space = np.max(points_rotated[:, 1]) - np.min(points_rotated[:, 1])
                
                # Calculăm "lățimea" pe axa X
                width_in_rotated_space = np.max(points_rotated[:, 0]) - np.min(points_rotated[:, 0])
                
                # Raportul aspect: Corp vertical are înălțime >> lățime
                aspect_ratio = height_in_rotated_space / (width_in_rotated_space + 0.001)
                
                # BONUS MARE pentru aspect vertical (corp în picioare = ratio > 2.0)
                if aspect_ratio > 2.0:
                    orientation_bonus = 0.5  # BONUS URIAȘ
                    ...
                    orientation_bonus = -0.3  # PENALIZARE pentru poziții culcate
                    print(f"         ❌ HORIZONTAL orientation! Aspect={aspect_ratio:.2f}")
                
                # BONUS SECUNDAR: Nas mai sus ca glezne ÎN IMAGINE (verificare slabă)
                ...
                print(f"         Final score: {score:.3f} (base={base_score:.2f}, orient={orientation_bonus:.2f})")
        ...
        # ============================================================
        # 2. EXTRAGERE KEYPOINTS (SECȚIUNEA CARE LIPSEA!)
        # ============================================================
        ...
        for name, idx in mapping.items():
            lm = landmarks[idx]
            # Conversie Imagine -> Spațiu Rotit
            u_px, v_px = lm.x * res, lm.y * res
            rot_x = (u_px - res/2) / scale + c_u
            rot_y = c_v - (v_px - res/2) / scale
            
            # Găsim Z-ul în spațiul rotit (Adâncimea)
            dist_sq = (best_points_rotated[:,0] - rot_x)**2 + (best_points_rotated[:,1] - rot_y)**2
            min_idx = np.argmin(dist_sq)
            rot_z = best_points_rotated[min_idx, 2] if dist_sq[min_idx] < 0.01 else np.mean(best_points_rotated[:, 2])
            
            # Rotim invers spre original
        ...
        # ============================================================
        # 3. SCALARE FINALĂ (SECȚIUNEA CARE LIPSEA!)
        # ============================================================
        # Calculăm înălțimea curentă a norului de puncte
        # Folosim punctele rotite, unde Y (axa 1) este verticala
        min_y = np.min(best_points_rotated[:, 1])
        max_y = np.max(best_points_rotated[:, 1])
        current_height = max_y - min_y
        
        if current_height <= 0: current_height = 1.0  # Evitare div by zero
        
        scaling_factor = real_height_meters / current_height
        
        print(f"   [SCALE] Cloud Height: {current_height:.3f} units -> Target: {real_height_meters:.3f}m")
        print(f"           Factor: {scaling_factor:.4f}")
        
        # Aplicăm scalarea pe toate punctele detectate
        ...
        # ============================================================
        # 4. PUNCTE DERIVATE (SECȚIUNEA CARE LIPSEA!)
        # ============================================================
        def mid(p1, p2):
